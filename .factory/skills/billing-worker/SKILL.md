# billing-worker SKILL

## Role
Handles quota calculation, billing session lifecycle, usage_log, privacy policy,
and token/user quota deduction.

## Scope
- `service/billing_session.go`, `service/quota.go`, `service/text_quota.go`
- `service/pre_consume_quota.go`, `service/log_info_generate.go`, `model/log.go`

## Rules
1. Read `pkg/billingexpr/expr.md` before touching billing expression code (CLAUDE.md Rule 7)
2. Privacy default: `STORE_FULL_TEXT_ENABLED=false` — never change this default
3. Quota deduction must be atomic (DB transactions or atomic updates)
4. `BillingSession.Settle` must handle pre-consume delta correctly
5. Use `common.Marshal`/`Unmarshal` (CLAUDE.md Rule 1)
6. Run `go test ./service/...` after every change
7. Check `validation-contract.md` assertions A11.x–A13.x before marking done
